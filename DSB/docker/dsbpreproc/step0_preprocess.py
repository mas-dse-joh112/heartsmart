...
def create_csv_data():
    ...
    row_no = 0
    print "create_csv_date", settings.BASE_PREPROCESSEDCSV_DIR + "dicom_data.csv"
    with open(settings.BASE_PREPROCESSEDCSV_DIR + "dicom_data.csv", "wb") as csv_file:
...
def enrich_dicom_csvdata():
    ...
    dicom_data = pandas.read_csv(settings.BASE_PREPROCESSEDCSV_DIR + "dicom_data.csv", sep=";")
    ...
    print 'dicom_data', dicom_data
    dicom_data = dicom_data.sort_values(["patient_id", "frame_no", "slice_location", "file_name"], ascending=[1, 1, 1, 1])
    ...
    dicom_data = dicom_data.sort_values(["patient_id", "frame_no", "slice_location_sort", "slice_location", "file_name"])
    ...
    dicom_data.to_csv(settings.BASE_PREPROCESSEDCSV_DIR + "dicom_data_enriched.csv", sep=";")

    dicom_data = dicom_data[dicom_data["frame_no"] == 1]
    dicom_data.to_csv(settings.BASE_PREPROCESSEDCSV_DIR + "dicom_data_enriched_frame1.csv", sep=";")
...
def enrich_traindata():
    ...
    dicom_data = pandas.read_csv(settings.BASE_PREPROCESSEDCSV_DIR + "dicom_data_enriched_frame1.csv", sep=";")
    ...
    enriched_traindata.to_csv(settings.BASE_PREPROCESSEDCSV_DIR + "train_enriched.csv", sep=";")


def get_patient_id(dir):
    print 'get patient id', dir
    parts = dir.split('/')
...
def get_slice_type(dir_name):
    print 'get slice type', dir_name
    parts = dir_name.split('/')
    print 'parts', dir_name, parts
...
def convert_sax_images(rescale=True, base_size=256, crop_size=256):
    ...
    for dicom_data in helpers.enumerate_sax_files():
        ...
        if file_count % 1000 == 0:
            print "enum sax",str(dicom_data.patient_id) + "\t" + str(dicom_data.rows) + "\t" + str(dicom_data.columns) + "\t" + str(dicom_data.series_number) + "\t" + str(dicom_data.spacing) + "\t"  + str(dicom_data.slice_thickness) + "\t" + str(dicom_data.sequence_name) + "\t" + str(dicom_data.image_position) + "\t" + str(dicom_data.slice_location) + "\t" + dicom_data.in_plane_encoding_direction
        ...
        print 'orig file_name', dicom_data.file_name, 'img_path', img_path
