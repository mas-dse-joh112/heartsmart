...
#         self.parallel_model.compile(optimizer = Adam(lr = 1e-4), loss = 'binary_crossentropy', metrics = [dice_coeff])
    ...
    def train_with_augmentation2(self, model_file, batch_size = 4, nb_epoch = 10 ):
        ...
        model_file = '/masvol/heartsmart/unet_model/data/combined_1_1_176_dice_aug_drop_32B.hdf5' #path to save the weights with best model
...
#             plt.show()
    ...
    def dump_and_sort(self):
        ...
        new_path = '/masvol/output/{0}/volume/1/1combined_1_1_176_dice_aug_drop_32B_cr/{1}_{2}_{3}_CR.json'.format(self.file_source,self.source_type,str(self.patient),self.image_size)
        print('one_file',self.image_one_file)
        ...
        for i in slicedict: #i is sax
            ...
            for j in slicedict[i]: #j is frame
                ...
                if zcount > zmax:
                    zmax = zcount
                    zmaxframe = j
            #print('zmaxframe',zmaxframe)
            #print('zminframe',zminframe)
    ...
    def clean_contours(self):
        ...
        pred_file_CR = "{0}/{1}/combined_1_1_176_dice_aug_drop_32B_predict_CR/{2}_{3}_{4}_CR_predictions_cleaned.npy".format(self.test_source_path,self.data_source,self.file_source,self.patient,self.image_size)
    ...
    def do_predict(self):
        ...
        pred_file = "{0}/{1}/combined_1_1_176_dice_aug_drop_32B_predict_CR/{2}_{3}_{4}_CR_predictions.npy".format(self.test_source_path,self.data_source,self.file_source,self.patient,self.image_size)
        np.save(pred_file, self.predictions)
        pred2 = np.round(self.predictions)
        pred_round_file = "{0}/{1}/combined_1_1_176_dice_aug_drop_32B_predict_CR/{2}_{3}_{4}_CR_pred_round.npy".format(self.test_source_path,self.data_source,self.file_source,self.patient,self.image_size)
        np.save(pred_round_file, pred2)
        ts_norm_file = "{0}/{1}/combined_1_1_176_dice_aug_drop_32B_predict_CR/{2}_{3}_{4}_CR_ts_norm.npy".format(self.test_source_path,self.data_source,self.file_source,self.patient,self.image_size)
...
if __name__ == "__main__":
    ...
    model_file = "/masvol/heartsmart/unet_model/data/combined_1_1_176_dice_aug_drop_32B.hdf5"
    ...
    mm.test_source_path = "/masvol/output/{0}/norm/1/1".format(mm.file_source)
    ...
    for i in glob.glob(inputpath):
        ...
        mm.image_one_file = "{0}/{1}/combined_1_1_176_dice_aug_drop_32B_models/{2}_{3}_{4}_one_count.json".format(mm.test_source_path, mm.data_source, mm.file_source, mm.patient, mm.image_size)
