...
#             plt.show()
    ...
    def dump_and_sort(self):
        count = 0
        origpath = '/masvol/data/{0}/{1}/{2}/study/'.format(self.file_source,source,str(self.patient))
        new_path = '/masvol/output/{0}/volume/{1}/{2}/{3}_{4}_{5}.json'.format(self.file_source,method,volume_path,self.source_type,str(self.patient),self.image_size)
...
#             print (min_max)
    ...
    def do_predict(self):
        ...
        pred_file = "{0}/{1}/{2}/{3}_{4}_{5}_predictions.npy".format(self.test_source_path,self.data_source,predict_path,self.file_source,self.patient,self.image_size)
        np.save(pred_file, self.predictions)
        pred2 = np.round(self.predictions)
        pred_round_file = "{0}/{1}/{2}/{3}_{4}_{5}_pred_round.npy".format(self.test_source_path,self.data_source,predict_path,self.file_source,self.patient,self.image_size)
        np.save(pred_round_file, pred2)
        ts_norm_file = "{0}/{1}/{2}/{3}_{4}_{5}_ts_norm.npy".format(self.test_source_path,self.data_source,predict_path,self.file_source,self.patient,self.image_size)
...
if __name__ == "__main__":
    ...
    image_size,source, source_type, method,Type, hdf5_path,predict_path,model_path,volume_path = arg

    file_source = "dsb"
    image_size = int(image_size)

    model_file = "/masvol/heartsmart/unet_model/{0}".format(hdf5_path)
    ...
    txt_file = "unet_model_{0}".format(source)
    mm.file_source = file_source
    mm.test_source_path = "/masvol/output/{0}/norm/{1}/{2}".format(mm.file_source,method, Type)
    mm.image_size = image_size

    inputpath = "/masvol/data/{0}/{1}/*".format(mm.file_source, source)
#     inputpath = "/masvol/output/{0}/norm/{1}/{2}/{3}/data/*".format(mm.file_source,method,Type,mm.source_type)
    pcount = 0
    print (inputpath)
    for i in glob.glob(inputpath):
        ...
        mm.patient = nodes[-1]
        print ('|'+mm.patient+'|')
        mm.image_4d_file = "{0}/{1}/data/{2}_{3}_{4}_{5}.npy".format(mm.test_source_path, mm.data_source, mm.file_source, mm.patient, mm.image_size,source)
        print ('4d',mm.image_4d_file)
        mm.image_source_file = "{0}/{1}/data/{2}_{3}_image_path.txt".format(mm.test_source_path, txt_file, mm.file_source, mm.patient)
        #print ('sf',mm.image_source_file)
        mm.image_one_file = "{0}/{1}/{2}/{3}_{4}_{5}_one_count.json".format(mm.test_source_path, mm.data_source, model_path, mm.file_source, mm.patient, mm.image_size)
