...
from keras.layers import Input, concatenate, Conv2D, MaxPooling2D, UpSampling2D, Dropout, Cropping2D, Activation, BatchNormalization
...
class myUnet(object):
    def __init__(self, model_name = "unet", nGPU=0, image_size = 256,  dropout = False, batchnorm = True, optimizer = 'Adam', lr=.00001, loss_fn="dice_loss"):
    ...
    def build_unet(self):
        ...
        '''
        print('-'*30)
        print ("Building U-net model with Batch Normalization")
        print('-'*30)
        ...
        if (self.nGPU > 0) :
            print ("compiling model for parallel GPU operation", self.nGPU)
            self.parallel_model = ModelMGPU(self.model, self.nGPU)
...
#                 print ("Prediction Scores before rounding", self.parallel_model.metrics_names, self.scores)

#                 pred2 = np.round(self.predictions)
#                 self.scores = self.parallel_model.evaluate (pred2,  self.test_labels, batch_size=4)
#                 #scores = self.model.evaluate (pred2,  self.test_labels, batch_size=4)
#                 print ("Prediction Scores after rounding", self.parallel_model.metrics_names, self.scores)
#             else :
...
#                 print ("Prediction Scores before rounding", self.model.metrics_names, self.scores)

#                 pred2 = np.round(self.predictions)
#                 #scores = self.parallel_model.evaluate (pred2,  self.test_labels, batch_size=4)
#                 self.scores = self.model.evaluate (pred2,  self.test_labels, batch_size=4)
#                 print ("Prediction Scores after rounding",self.model.metrics_names, self.scores)
    ...
    def train_and_predict(self, model_path, batch_size = 4, nb_epoch = 10, augmentation = False): 
        ...
        if (self.nGPU > 0) :
            ...
            self.predictions = self.model.predict(self.test_images, batch_size=1, verbose=1)
            self.scores = self.model.evaluate (self.predictions, self.test_labels, batch_size=4)
            print ("Prediction Scores", self.model.metrics_names, self.scores)
    ...
    def save_model_info(self, mypath = "./"):
        ...
        try:
            ...
            if (self.nGPU > 0) :
                ...
                for i in range (len(self.scores)):
                    hist['eval_'+ self.model.metrics_names[i]] = self.scores[i]
        ...
        if (self.nGPU > 0) :
            ...
            for i in range (len(self.scores)):
                perf['eval_'+ self.model.metrics_names[i]] = self.scores[i]
...
# def train_unet_model (model_name, GPUs, image_size, training_images, training_labels, test_images, test_labels, model_path, dropout, optimizer, learningrate, lossfun, batch_size, epochs, augmentation = False, model_summary = False):
...
#         print (" enter model_name, GPUs, image_size, training_images, training_labels, test_images, test_labels, model_path, dropout (True or False), optimizer, learningrate, loss_function, batch_size, epochs, augmentation (True or False), model_summary (True or False)")
#         sys.exit() 
    
#     model_name = sys.argv[1]
#     GPUs = int(sys.argv[2])
#     image_size = int(sys.argv[3])
#     training_images = sys.argv[4]
#     training_labels = sys.argv[5]
#     test_images = sys.argv[6]
#     test_labels = sys.argv[7]
