...
class myUnet(object):
    ...
    def train_and_predict(self, model_path, batch_size = 4, nb_epoch = 10, augmentation = False): 
        ...
        print('-'*30)
        print('predict test data....')
        self.predictions = self.parallel_model.predict(self.test_images, batch_size, verbose=1)
        #self.predictions = self.model.predict(self.test_images, batch_size=1, verbose=1)
        self.scores = self.parallel_model.evaluate (self.predictions, self.test_labels, batch_size)
