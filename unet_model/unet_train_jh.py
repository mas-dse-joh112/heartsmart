...
class myUnet(object):
    def __init__(self, model_name = "unet", image_size = 256, model_type = "small", batch_size = 4, learning_rate = 0.0001, dropout = 0):
    ...
    def build_unet_small(self):
        ...
        self.parallel_model.compile(optimizer=RMSprop(lr=self.learning_rate), loss=bce_dice_loss, metrics=[dice_coeff])
    ...
    def build_unet(self):
        ...
        #self.parallel_model.compile(optimizer=RMSprop(lr=0.0001), loss=penalized_bce_loss(weight=0.08), metrics=['binary_accuracy'])
        #self.parallel_model.compile(optimizer=RMSprop(lr=0.0001), loss=dice_loss, metrics=[dice_coeff])

        #metrics=['accuracy'] calculates accuracy automatically from cost function. So using binary_crossentropy shows binary 
        #accuracy, not categorical accuracy.Using categorical_crossentropy automatically switches to categorical accuracy
        #One can get both categorical and binary accuracy by using metrics=['binary_accuracy', 'categorical_accuracy']
        
        self.parallel_model.compile(optimizer = Adam(lr = self.learning_rate), loss = dice_loss, metrics = [dice_coeff])
        #self.parallel_model.compile(optimizer = Adam(lr = lr), loss = 'binary_crossentropy', metrics = [dice_coeff])
        #self.parallel_model.compile(optimizer = Adam(lr = lr), loss = my_bce_loss, metrics = ['binary_accuracy'])
    ...
    def train_and_predict(self, model_path, nb_epoch = 10, augmentation = False): 
        ...
        print('Fitting model...')
        print('-'*30)
        self.history = self.parallel_model.fit(self.train_images, self.train_labels, self.batch_size, nb_epoch, verbose=1,validation_split=0.2, shuffle=True, callbacks=[model_checkpoint])

        print('-'*30)
        print('predict test data....')
        self.predictions = self.parallel_model.predict(self.test_images, self.batch_size, verbose=1)
        self.scores = self.parallel_model.evaluate(self.predictions, self.test_labels, self.batch_size)
    ...
    def train_with_augmentation(self, model_file, batch_size = 4, nb_epoch = 10 ):
        ...
        for img_tr, mask_tr in train_generator:
                ...
                self.parallel_model.fit(img_tr, mask_tr, batch_size, nb_epoch, verbose=1,validation_split=0.2, shuffle=True, callbacks=[model_checkpoint])
                augmentation_round += 1
                if (augmentation_round == MAX_AUG):
                      break
...
def train_unet_model(model_name, image_size, training_images, training_labels, test_images, test_labels, model_path,  batch_size, epochs, augmentation = False, model_summary = False, learning_rate = 0.0001, dropout = 0):
    ...
    myunet = myUnet(model_name, image_size, "large", batch_size, learning_rate, dropout)
    myunet.load_data(train_data, test_data)

    if (model_summary == True):
        myunet.model.summary()
        
    res = myunet.train_and_predict(model_path, nb_epoch = epochs, augmentation = augmentation)
...
if __name__ == "__main__":
    ...
    if len(args) != 13:
        ...
        print (" enter model_name, image_size, training_images, training_labels, test_images, test_labels, model_path,  batch_size, epochs, augmentation, model_summary, learning_rate, dropout")
        sys.exit() 
    ...
    mymodel = train_unet_model(model_name, image_size, training_images, training_labels, test_images, test_labels, model_path,  batch_size, epochs, augmentation, model_summary, learning_rate, dropout)
